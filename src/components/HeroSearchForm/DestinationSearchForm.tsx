...
export const DestinationSearchForm = ({ className, formStyle = 'default' }: Props) => {
  const router = useRouter()

  // Prefetch the destination categories page to improve performance
  useEffect(() => {
    router.prefetch('/destination-categories/all')
  }, [router])
  ...
  const handleFormSubmit = (formData: FormData) => {
    ...
    let url = '/destination-categories/all'
    if (location) {
